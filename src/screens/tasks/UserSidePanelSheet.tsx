    ...
    <View style={[styles.flex, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.userHeader}>
          <View
            style={[
              styles.avatarPlaceholder, { backgroundColor: theme.surface, borderColor: ROLE_COLORS[role] }
            ]}>
            <Text style={[styles.avatarText, { color: ROLE_COLORS[role] }]}>
              {name.charAt(0).toUpperCase()}
            </Text>
          </View>
          <View>
            <Text style={[styles.name, { color: theme.text }]}>{name}</Text>
            <Text style={[styles.role, { color: ROLE_COLORS[role] }]}>
              {ROLE_LABELS[role]}
            </Text>
          </View>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          <Text style={[styles.cardTitle, { color: theme.textMuted }]}>Current Status</Text>
          <View
            style={[
              styles.statusBadge,
              {
                borderColor: syncColor + "40",
                backgroundColor: syncColor + "15",
              },
            ]}>
          ...
          <Text style={[styles.timeLabel, { color: theme.textMuted }]}>
            Updated {dayjs(lastUpdated).format("MMM D, h:mm A")}
          </Text>

          {notes && (
            <View style={[styles.notesBox, { borderTopColor: theme.border }]}>
        ...
        <View style={styles.row}>
          <View style={[styles.card, styles.halfCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.textSecondary }]}>Time Logged</Text>
            <Text style={[styles.statValue, { color: theme.text }]}>{timeLogged}m</Text>
          </View>
          <View style={[styles.card, styles.halfCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.textSecondary }]}>Milestones</Text>
            <Text style={[styles.statValue, { color: theme.text }]}>{milestonesCompleted}</Text>
          </View>
        </View>
        ...
        {canEditRole && (
          <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.textSecondary }]}>Change Role</Text>
            <View style={styles.roleGrid}>
              {availableRoles.map((r) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.roleCard, { backgroundColor: theme.background, borderColor: theme.border },
                      role === r && {
                        borderColor: ROLE_COLORS[r],
                        backgroundColor: `${ROLE_COLORS[r]}1A`,
                      },
                    ]}
                    onPress={() => handleChangeRole(r)}>
                    <Text
                      style={[
                        styles.roleCardText, { color: theme.textSecondary },
        ...
        <View style={styles.actionsBox}>
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.surface, borderColor: theme.border }]} onPress={() => {}}>
